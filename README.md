...
1. 若 `.local/CURRENT.md` 存在，且当前时间距“最后聚合时间”不足 **60 分钟**：不执行 Git 操作，直接读取并遵守该文件。
2. 若文件不存在或已经达到 60 分钟：先检查工作区是否有已跟踪文件的未提交修改；没有则执行 `git pull --ff-only`，再重新生成聚合文件。
...
当 `memory-save` 或 `memory-new` 已成功提交并推送 `common/*.md` 的改动时，不必重新聚合整个缓存。应立即将改动同步到 `.local/CURRENT.md`：

1. 已有主题文档：把新增规则追加到该文档对应的“通用规则”章节；如果改动修改、删除或重排了既有内容，则用源文档完整替换该章节，避免缓存与源文件不一致。
2. 新主题文档：在工具专属补充之前新增该文档的完整“通用规则”章节。
3. 写入“最后增量同步时间”和“最后增量同步的 Git 提交号”。增量同步属于缓存生成流程，不提交、不推送 `.local/CURRENT.md`。

完整聚合仍仅用于首次初始化、缓存达到 60 分钟，或增量同步无法可靠定位对应章节时。
